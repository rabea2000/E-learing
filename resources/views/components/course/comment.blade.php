...
<article class='{{$class}}'>
  <footer class="flex justify-between items-center mb-2 ">
    <div class="flex items-center">
      <p class="inline-flex items-center ml-3 text-sm text-gray-900  font-semibold">
        <img class="ml-2 w-6 h-6 rounded-full" src="{{ asset('storage/'.$comment->user->img_url) }}"
    ...
    <div x-data="{ showModal: false}" class="relative">
      <button @click="showModal = true" id="dropdownCommentButton"
  ...
  <div class="flex items-center mt-4 space-x-4 " dir="ltr">
    ...
    <div  x-data="{ showReplyModal: false }">
      ...
      <x-course.replyinput x-show="showReplyModal" x-cloak @click.away="showReplyModal = false" action="/reply"
        :comment="$comment" />
    </div>
    @endif
...
{{-- class="flex items-center py-2.5 px-4 text-xs font-medium text-center text-white bg-blue-700 rounded-lg focus:ring-4
